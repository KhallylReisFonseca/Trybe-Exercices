...
const createMenu = (menu) => {
  ...
  objetoFinal.order = (string) => {
  if (Object.keys(objetoFinal.fetchMenu().food).includes(string)) {
    objetoFinal.consumption.push(string);
  } else if (Object.keys(objetoFinal.fetchMenu().drink).includes(string)) {
    objetoFinal.consumption.push(string);
  } else {
  ...
  objetoFinal.pay = () => {
   let total = 0;
   const consumo = objetoFinal.consumption;
   for (let key of consumo) {
    if (Object.keys(objetoFinal.fetchMenu().food).includes(key)) {
      const value = Number(objetoFinal.fetchMenu().food[key]);
      total += value;
    } else {
      total += objetoFinal.fetchMenu().drink[key];
    }
   }
   return total * 1.1;
  };
